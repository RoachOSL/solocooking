...
Shared instructions for every AI coding assistant working in this repository.
`CLAUDE.md` and `AGENTS.md` are thin pointers to this file — edit the rules
here, in one place, and both tools stay in sync.
...
- `ai-guidelines/PROJECT_NOTES.md`

Follow both files as durable project guidance. Reusable engineering and
architecture rules belong in `ai-guidelines/ARCHITECTURE.md`; SoloCooking-specific
domain and product decisions belong in `ai-guidelines/PROJECT_NOTES.md`.
...
- When a reusable architectural or engineering decision is agreed, record it
  in `ai-guidelines/ARCHITECTURE.md`. Record project-specific decisions in
  `ai-guidelines/PROJECT_NOTES.md`.
...
- `ai-guidelines/ARCHITECTURE.md` is binding. Follow every decision recorded
  there; never silently deviate from it or change established direction. When
  you believe a recorded decision is wrong, raise it and ask first — do not
  change it, or act against it, without asking.
- When the user asks a question and the reply grows into a real explanation (a
  ...
  `ai-notes/LEARNINGS.md` as a dated entry and tell the user. Create the file if
...
When the user asks for a code review, prefer a dedicated code-review skill or
command if one is available to you; otherwise perform the most thorough review
available. Do not, on your own, invoke any cloud-based, multi-agent, or
quota-limited review variant — those run only when the user explicitly asks for
them. Treat `ai-guidelines/ARCHITECTURE.md`, `ai-guidelines/PROJECT_NOTES.md`, and all
repository instructions as binding. Prioritize actionable defects — correctness,
regressions, security, architecture or contract violations, and missing tests —
over summaries and subjective style preferences. Report findings in severity
order with precise file and line references. If no findings remain, say so
explicitly and note any residual risks or testing gaps.
